...
---
###3.2.2. 가설_집권당(보수당, 진보당)의 정치성향이 범죄율에 영향을 미칠 것이다. - T검증  
```{r}
library(dplyr)
library(tidyr)
library(magrittr)  
...
```
<분산 동질성 검정결과>  
p-value =0.3791 >0.05, 95% 신뢰도에서 신뢰구간 [0.1181087,2.23212838]에 1이 존재한다. 따라서 두 집권정당의 범죄율 분산이 동일하다는 것을 알 수 있다. 두 집단의 분산이 같다는 가정으로 T-검증을 수행할 것이다.  
...
```
<독립표본 T검증분석결과>    
p-value =0.4268 >0.05이며, 95% 신뢰도에서 신뢰구간 [-0.1937625,0.4359847]에 0이 존재한다. 따라서 집권당에 따라 범죄율에 차이가 없다는 귀무가설을 기각하지 못한다. 

```{r}
#정당에 따른 범죄율 기술 통계량  
library(psych)  
describeBy(T_crime1$범죄율, T_crime1$집권정당)  
...
```
###3.2.3. 가설_집권당(보수당, 진보당)의 정치성향이 자살률에 영향을 미칠 것이다.  (단위는 10만명당)
```{r}
# 2002-2019년 자살률 데이터 수집&정제하기  
...
<독립표본 T검증분석결과>  
p-value =0.001269 < 0.05이며, 95% 신뢰도에서 신뢰구간 [2.121143, 7.167746]사이에 0이 존재하지 않는다. 따라서 집권당에 따라 자살률에 차이가 없다는 대립가설을 채택할 수 있다.  
