...
#### 1.2 연구방법 및 전제
- 왜 우리가 민주당 국힘, 진보 보수만 있는 데이터를 ㅣ용해 했는지..
...
##### 워드클라우드 
###### 데이터수집
트위터에서 보수당,진보당에 관해 사람들이 갖고 있는 생각이 궁금했습니다. 따라서 수업시간에 배운 트위터 데이터를 통해 사람들이 현재 보수당인 국민의 힘, 진보당인 더불어 민주당을 해시태그로하여 텍스트를 가져왔습니다. 
...
```
트윗 결과 중 텍스트 부분만 가져와 명사를 추출하고 두 글자 이상 단어만 뽑아냈습니다.
...
```
gsub와 tm_map을 활용해 필요없는 단어들을 지워주었습니다.
```{r}
...
wordDf <- data.frame( word = names(x = wordFreq),
                              ...
                              row.names = NULL) 
```
등장하는 단어들의 빈도수를 행렬로 표현해 열 합계를 구해줬습니다.
```{r}
# 워드클라우드로 나타내기 
...
```  
부정 단어는 -1로, 긍정단어는 1로 출력하게하고 긍정,부정 이외의 단어는 NA값으로 나오게 하여 긍정,부정 결과만 나올 수 있게 해주었습니다.
```{r}
...
```
깃허브에서 군산대 감성사전의 긍정,부정어를 다운받아 각각 positive,negative변수에 불러왔습니다.
...
```
파이차트 결과 진보의 경우 긍정단어가 비중이 높게 나왔습니다. 
...
```
파이차트 결과 보수는 부정적인 단어의 비중이 더 높게나왔습니다.
